...
class TinyImageNet200(DataHandler):
    ...
    def set_custom_transforms(self, transforms_dict):
        ...
        if transforms_dict.get("crop"):
            logger.LOGGER.info("Crop with padding of {}".format(transforms_dict.get("crop")))
...
class AuxDataset(data.Dataset):
    def __init__(self, img_data, transform):
        self.x_trn = img_data['x']
        self.y_trn = img_data['y']
        self.transform = transform

    def __getitem__(self, index):
        x = Image.fromarray(self.x_trn[index])
        x = self.transform(x)
        out = {'images': x, 'labels': self.y_trn[index]}
        return out
...
def get_imagenet_32_datasets(dataset, path):
    ...
    return AuxDataset(trn_data, transform=trsf)
